import { client, urlFor } from '@/lib/sanity'
import Image from 'next/image'
import Link from 'next/link'

async function getArticles() {
  return await client.fetch(`
    *[_type == "post"] | order(publishedAt desc) {
      _id,
      title,
      slug,
      publishedAt,
      summary,
      coverImage
    }
  `)
}

export default async function ArticlesPage() {
  const articles = await getArticles()

  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '2rem 2rem 4rem 2rem' }}>
      <h1 style={{ fontSize: '2.5rem', fontWeight: '300', color: '#1a1a1a', marginBottom: '2rem' }}>
        All Articles
      </h1>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        {articles.length === 0 ? (
          <p style={{ color: '#9ca3af' }}>No articles yet.</p>
        ) : (
          articles.map((article: any) => (
            <Link key={article._id} href={`/articles/${article.slug?.current}`} style={{ textDecoration: 'none' }}>
              <div style={{ 
                display: 'flex', 
                gap: '1.5rem', 
                borderBottom: '1px solid #e5e7eb', 
                paddingBottom: '1.5rem',
                alignItems: 'flex-start'
              }}>
                {article.coverImage && (
                  <div style={{ position: 'relative', width: '150px', height: '100px', borderRadius: '0.5rem', overflow: 'hidden', flexShrink: 0 }}>
                    <Image src={urlFor(article.coverImage).url()} alt={article.title} fill style={{ objectFit: 'cover' }} />
                  </div>
                )}
                <div>
                  <h2 style={{ fontSize: '1.3rem', fontWeight: '400', color: '#1a1a1a' }}>
                    {article.title}
                  </h2>
                  {article.summary && <p style={{ color: '#4b5563', fontSize: '0.95rem' }}>{article.summary}</p>}
                  <p style={{ color: '#9ca3af', fontSize: '0.8rem', marginTop: '0.5rem' }}>
                    {article.publishedAt
                      ? new Date(article.publishedAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })
                      : ''}
                  </p>
                </div>
              </div>
            </Link>
          ))
        )}
      </div>
    </div>
  )
}